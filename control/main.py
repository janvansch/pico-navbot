...
def calc_target_pos(deviation_angle, detour_distance, target_distance, obstacle_side):
    ...
    if deviation_angle == 180:
        ...
        current_target_distance = target_distance + detour_distance
    
    else:
        # target distance = distance to target from detour position
        current_target_distance = math.sqrt(target_distance**2 + detour_distance**2 - 2 * target_distance * detour_distance * math.cos(math.radians(deviation_angle))) 
    
    print(":--> Target distance: ", current_target_distance)
          
    # calc detour position angle
    detour_position_angle = math.degrees(math.acos((current_target_distance**2 + detour_distance**2 - target_distance**2) / (2 * current_target_distance * detour_distance)))
...
def go_target(target_route):
    ...
    for leg in target_route:
        ...
        print("--> Leg number: ", leg_num, " Heading: ", leg_heading, " Distance: ", leg_distance_cm, " cm")
...
ir_front_left.irq(handler=set_front_obstacle, trigger=Pin.IRQ_FALLING, hard=True) # obstacle
ir_front_centre.irq(handler=set_front_obstacle, trigger=Pin.IRQ_FALLING, hard=True) # obstacle
ir_front_right.irq(handler=set_front_obstacle, trigger=Pin.IRQ_FALLING, hard=True) # obstacle
